...
namespace Ellis {
	...
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "Ellis.Vector2", ScriptFieldType::Vector2 },
		{ "Ellis.Vector3", ScriptFieldType::Vector3 },
		{ "Ellis.Vector4", ScriptFieldType::Vector4 },

		{ "Ellis.Entity", ScriptFieldType::Entity }
	};
	...
	namespace Utils {
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				EL_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::Init()
	{
		...
		bool status = LoadAssembly("Resources/Scripts/Ellis-ScriptCore.dll");
		if (!status) 
		{
			EL_CORE_ERROR("[ScriptEngine] Could not load Ellis-ScriptCore assembly");
			return;
		}
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		MonoClass* entityClass = mono_class_from_name(s_Data->CoreAssemblyImage, "Ellis", "Entity");
		for (int32_t i = 0; i < numTypes; i++)
		{
			uint32_t cols[MONO_TYPEDEF_SIZE];
			mono_metadata_decode_row(typeDefinitionsTable, i, cols, MONO_TYPEDEF_SIZE);
			...
			EL_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					EL_CORE_WARN("   {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
	...
	{
		EL_CORE_ASSERT(s_Data->EntityInstances.find(uuid) != s_Data->EntityInstances.end());
