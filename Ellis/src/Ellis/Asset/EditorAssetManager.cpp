...
#include <Ellis/Asset/AssetImporter.h>
...
namespace Ellis {
	...
	static AssetType GetAssetTypeFromFileExtension(const std::filesystem::path& extension)
	{
		...
		{
			EL_CORE_WARN("Could not find AssetType for {}", extension);
	...
	Ref<Asset> EditorAssetManager::GetAsset(AssetHandle handle)
	{
		...
		else
		{
			...
			if (!asset)
				EL_CORE_ERROR("EditorAssetManager::GetAsset - asset import failed!");
	...
	void EditorAssetManager::ImportAsset(const std::filesystem::path& filepath)
	{
		...
		EL_CORE_ASSERT(metadata.Type != AssetType::None);
	...
	bool EditorAssetManager::DeserializeAssetRegistry()
	{
		...
		{
			EL_CORE_ERROR("Failed to load .eproj file \"{0}\"\n     {1}", path, e.what());
			return false;
		}
