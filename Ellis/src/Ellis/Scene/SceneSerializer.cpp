...
namespace YAML {
	...
	template<>
	struct convert<Ellis::UUID>
	{
		static Node encode(const Ellis::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Ellis::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)                          \
	...
	{
		EL_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		EL_CORE_TRACE("Deserializing scene \"{0}\"", sceneName);
				...
				if (scriptComponent)
				{
					...
					if (scriptFields)
					{
						...
						for (auto scriptField : scriptFields)
						{
							...
							EL_CORE_ASSERT(fields.find(name) != fields.end());
