...
void Sandbox2D::OnUpdate(Ellis::Timestep ts)
{
...

	{
		...
		Ellis::Renderer2D::DrawRotatedQuad({ 1.0f, 0.0f }, { 0.8f, 0.8f }, -45.0f, { 0.8f, 0.2f, 0.3f, 1.0f });
		...
		Ellis::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckerboardTexture, 10.0f);
		Ellis::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, rotation, m_CheckerboardTexture, 20.0f);
		...
		for (float y = -5.0f; y < 5.0f; y += 0.5f)
		{
			...
			{
				glm::vec4 color = { (x + 5.0f) / 10.0f, 0.4f, (y + 10.0f) / 20.0f, 0.7f };
				Ellis::Renderer2D::DrawQuad({ x, y }, { 0.45f, 0.45f }, color);
			}
		}
		Ellis::Renderer2D::EndScene();
	}
...
void Sandbox2D::OnImGuiRender()
{
	...
	auto stats = Ellis::Renderer2D::GetStats();
