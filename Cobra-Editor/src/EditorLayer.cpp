...
#include <Cobra/Scripting/ScriptGlue.h>
#include <Cobra/Renderer/Font.h>
#include <Cobra/Asset/TextureImporter.h>
#include <Cobra/Asset/SceneImporter.h>
#include <Cobra/Asset/AssetManager.h>
...
namespace Cobra {
	...
	void EditorLayer::OnAttach()
	{
		...
		else
		{
			...
			// if no project is open close Cobra
			if (!OpenProject())
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(CB_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
		dispatcher.Dispatch<KeyTypedEvent>(CB_BIND_EVENT_FN(EditorLayer::OnKeyTyped));
		dispatcher.Dispatch<MouseButtonPressedEvent>(CB_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
		dispatcher.Dispatch<WindowDropEvent>(CB_BIND_EVENT_FN(EditorLayer::OnWindowDrop));
	}
	...
	{
		//std::string filepath = FileDialogs::OpenFile("Cobra Scene (*.cobra)\0*.cobra\0");
		//if (!filepath.empty())
		//	OpenScene(filepath);
	}
	...
	{
		std::string filepath = FileDialogs::SaveFile("Cobra Scene (*.cobra)\0*.cobra\0");
		if (!filepath.empty())
		{
	...
	{
		CB_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
	...
	void Cobra::EditorLayer::OnDuplicateEntity()
	{
		if (m_SceneState != SceneState::Edit)
			return;
