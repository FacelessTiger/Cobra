...
#include <Cobra/Core/FileSystem.h>
...
namespace Cobra {
	...
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "Cobra.Vector2", ScriptFieldType::Vector2 },
		{ "Cobra.Vector3", ScriptFieldType::Vector3 },
		{ "Cobra.Vector4", ScriptFieldType::Vector4 },

		{ "Cobra.Entity", ScriptFieldType::Entity }
	};
	...
	namespace Utils {
		...
		static MonoAssembly* LoadMonoAssembly(const std::filesystem::path& assemblyPath, bool loadPDB = false)
		{
			...
			if (loadPDB)
			{
				...
				if (std::filesystem::exists(pdbPath))
				{
					...
					CB_CORE_INFO("Loaded PDB {}", pdbPath);
				}
			}
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				CB_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::Init()
	{
		...
		bool status = LoadAssembly("Resources/Scripts/Cobra-ScriptCore.dll");
		if (!status) 
		{
			CB_CORE_ERROR("[ScriptEngine] Could not load Cobra-ScriptCore assembly");
			return;
		}
		...
		if (!status)
		{
			CB_CORE_ERROR("[ScriptEngine] Could not load app assembly");
			return;
		}
		...
		s_Data->EntityClass = ScriptClass("Cobra", "Entity", true);
	}
	...
	void ScriptEngine::InitMono()
	{
		...
		MonoDomain* rootDomain = mono_jit_init("CobraJITRuntime");
		CB_CORE_ASSERT(rootDomain);
	...
	{
		s_Data->AppDomain = mono_domain_create_appdomain("CobraScriptRuntime", nullptr);
	...
	void ScriptEngine::OnUpdateEntity(Entity entity, Timestep ts)
	{
		...
		{
			CB_CORE_ERROR("Could not find ScriptInstance for entity {}", entityUUID);
		}
	}
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		MonoClass* entityClass = mono_class_from_name(s_Data->CoreAssemblyImage, "Cobra", "Entity");
		for (int32_t i = 0; i < numTypes; i++)
		{
			uint32_t cols[MONO_TYPEDEF_SIZE];
			mono_metadata_decode_row(typeDefinitionsTable, i, cols, MONO_TYPEDEF_SIZE);
			...
			std::string fullName = (strlen(nameSpace) == 0) ? className : fmt::format("{}.{}", nameSpace, className);
			...
			CB_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					CB_CORE_WARN("   {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
	...
	{
		CB_CORE_ASSERT(s_Data->EntityInstances.find(uuid) != s_Data->EntityInstances.end());
