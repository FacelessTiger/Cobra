...
namespace YAML {
	...
	template<>
	struct convert<Cobra::UUID>
	{
		static Node encode(const Cobra::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, Cobra::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)                          \
	...
	static std::string Rigidbody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		CB_CORE_ASSERT(false, "Unknown body type");
		return {};
	}
	...
	{
		CB_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	bool SceneSerializer::Deserialize(const std::filesystem::path& filepath)
	{
		...
		{
			CB_CORE_ERROR("Failed to load .cobra file \"{0}\"\n     {1}", filepath, e.what());
			return false;
		}
					...
					if (scriptFields)
					{
						...
						if (entityClass)
						{
							...
							for (auto scriptField : scriptFields)
							{
								...
								CB_CORE_ASSERT(fields.find(name) != fields.end());
